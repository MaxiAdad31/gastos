...
def agregar_ingreso():
    if request.method == 'POST':
        nuevo_ingreso = Ingreso(
            importe=float(request.form['importe']),  # Cambiado de 'cantidad' a 'importe'
            concepto=request.form['concepto'],  # Cambiado de 'descripcion' a 'concepto'
            fecha=datetime.strptime(request.form['fecha'], '%Y-%m-%d').date(),
            detalle=request.form.get('detalle')  # Nuevo campo
        )
        db.session.add(nuevo_ingreso)
        db.session.commit()
        flash('Ingreso agregado exitosamente', 'success')
        return redirect(url_for('listar_ingresos'))
    return render_template('ingresos/agregar.html')

@app.route('/ingresos/editar/<int:id>', methods=['GET', 'POST'])
def editar_ingreso(id):
    ingreso = Ingreso.query.get_or_404(id)
    if request.method == 'POST':
        ingreso.importe = float(request.form['importe'])  # Cambiado
        ingreso.concepto = request.form['concepto']  # Cambiado
        ingreso.fecha = datetime.strptime(request.form['fecha'], '%Y-%m-%d').date()
        ingreso.detalle = request.form.get('detalle')  # Nuevo campo
        db.session.commit()
